...
| `textDocument/codeAction` visibility and promotion refactors | Partial | Changes visibility for methods, properties, constants, and promoted properties. Promotes simple constructor assignment patterns to constructor property promotion with safety guards. |
| `textDocument/codeAction` PHPDoc signature sync | Partial | Updates `@param` order/types and native-return-driven `@return` tags from function/method signatures. Preserves summaries and unrelated tags such as templates, throws, deprecation, virtual properties, and virtual methods. |
