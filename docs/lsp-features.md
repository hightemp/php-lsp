...
| `textDocument/codeAction` extract and inline refactors | Partial | Extracts exact selected expressions to local variables, extracts class-scope scalar literals to `private const`, and inlines local variables with one simple assignment and one same-block read. Edits are resolved lazily and stale document versions resolve to a no-op edit. |
