...
|---|---|---|---|---|
| Hover | `src/lsp/hover.rs`, shared parameter/type Markdown helpers in `src/lsp/completion_helpers.rs` | `resolve.rs`, PHPDoc helpers | `workspace.rs` symbol lookup plus `SymbolInfo` source ranges for FQN/source metadata | `tests/e2e_hover.rs` |
