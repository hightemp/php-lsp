...
|---|---|---|---|---|
| Hover | `src/lsp/hover.rs`, shared parameter/type Markdown helpers in `src/lsp/completion_helpers.rs` | `resolve.rs`, PHPDoc helpers | `workspace.rs` symbol lookup plus `SymbolInfo` source ranges, relations, templates, and bindings | `tests/e2e_hover.rs` |
