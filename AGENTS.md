...
- Памяти на компе мало. При запуске сборки это надо учитывать. Можно собирать всё в один поток.
- Перед тем как сделать задачу помечай что будешь делать в TASKS.md.
- После выполнения задачи отмечай в TASKS.md.
- При выполнении задач не должно быть хардкода
- не используй mgrep
- Используй MCP LSP serena для поиска по rust коду и рефакторинга, если доступна.
   Можешь так же использовать semantic-search-mcp и codegraph для поиска по коду.
- Если что-то не понятно или есть вопросы можешь воспользоваться mcp context7 для поиска по документациям проектов или воспользоваться веб-поиском.
- Перед тем как сдать задачу с исправлениями кода запусти субагента Verifier чтобы он все проверил. Исправь все что он напишет и запускай его снова пока все не будет исправлено.
- Не забывай обновлять документацию, но не надо нести в документацию все правки из задач, только новые фичи
- На последним этапе перед тем как сдать задачу проверь что все задукоментировано

## Нужно учесть

- Что в vendor папке PHP проекта могут быть симлинки на разрабатываемые библиотеки. Так что не стоит пытаться убрать симлинки.
...
- Register a replacement indexing run before publishing replacement runtime state or clearing/rebuilding Composer, vendor, stub, cache, or aggregate state. Carry that same reserved guard through root-stub staging and the workspace runner; an older caller whose reserved lease became stale must stop rather than create another run. Route workspace/cache/vendor/Twig/open-document/diagnostic commits through its `IndexingRunLease`; keep the owning guard alive through final post-processing and `ready` publication.
