...
5. Используй MCP LSP serena для поиска по rust коду и рефакторинга
   Можешь так же использовать semantic-search-mcp и codegraph для поиска по коду.
6. Если что-то не понятно или есть вопросы можешь воспользоваться mcp context7 для поиска по документациям проектов или воспользоваться веб-поиском.
7. Не забывай обновлять документацию, но не надо нести в документацию все правки из задач, только новые фичи
8. Перед тем как сдать задачу с исправлениями кода запусти субагента Verifier чтобы он все проверил. Исправь все что он напишет и запускай его снова пока все не будет исправлено.
