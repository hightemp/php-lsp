...
#[tokio::test(flavor = "current_thread")]
async fn test_variable_completion_scope_edge_cases() {
    let (mut service, socket) = LspService::new(PhpLspBackend::new);
    tokio::spawn(async move {
        socket.collect::<Vec<_>>().await;
    });
    ...
    let code_with_markers = r#"<?php
function declared(string $functionParam): void {}
$globalLocal = 1;
$/*global*/;

class Subject {
    public function __construct(private string $promotedName) {
        $pro/*promoted*/;
    }

    public function methodOne(string $methodSharedOne): void {
        $method/*methodOne*/;
    }

    public function methodTwo(string $methodSharedTwo): void {
        $method/*methodTwo*/;
    }

    public function instanceThis(): void {
        $th/*instanceThis*/;
    }

    public static function staticThis(): void {
        $th/*staticThis*/;
    }

    public function nestedThis(): void {
        $closure = function (): void {
            $th/*closureThis*/;
        };
        $staticClosure = static function (): void {
            $th/*staticClosureThis*/;
        };
    }
}
...
function captures(
    string $captured,
    string $captureMissing,
    string &...$captureRest,
): void {
    $captureOuterLocal = 1;
    $captureR/*byRefRest*/;

    $closure = function (bool $closureOwn) use ($captured): void {
        $c/*closureUse*/;
    };

    $arrowOuter = 1;
    $arrow = fn (bool $arrowOwn): mixed => $a/*arrowCapture*/;

    $nestedClosure = function (bool $nestedClosureParam) use ($captured): void {
        $nestedVisible = 1;
        $nestedArrow = fn (bool $nestedArrowParam): mixed => $n/*nestedArrow*/;
    };

    $outerArrow = fn (bool $nestedArrowLayer): mixed =>
        function (bool $nestedClosureLayer) use ($nestedArrowLayer): void {
            $n/*nestedClosure*/;
        };
}

function incomplete(string $unicodeParam, string $incompleteParam): void {
    echo "😀"; $unicodeLocal = 1; $uni/*unicode*/; $unicodeAfter = 2;
    $inc/*incomplete*/;
"#;
    let markers = [
        "/*global*/",
    ...
    let uri = "file:///test/variable-scope-edges.php";
    service
        .ready()
        .await
        .unwrap()
    ...
    let cases: &[(&str, &[&str], &[&str])] = &[
        (
            "/*global*/",
            &["$globalLocal"],
            &["$functionParam", "$promotedName", "$this"],
        ),
        ("/*promoted*/", &["$promotedName"], &["$methodSharedOne"]),
        (
            "/*methodOne*/",
            &["$methodSharedOne"],
            &["$methodSharedTwo"],
        ...
        ("/*staticThis*/", &[], &["$this"]),
        ("/*closureThis*/", &["$this"], &[]),
        ("/*staticClosureThis*/", &[], &["$this"]),
        ("/*byRefRest*/", &["$captureRest"], &[]),
        (
            "/*closureUse*/",
            &["$captured", "$closureOwn"],
            &["$captureMissing", "$captureRest", "$captureOuterLocal"],
        ),
        (
            "/*arrowCapture*/",
            &["$arrowOuter", "$arrowOwn"],
        ...
        (
            "/*nestedArrow*/",
            &["$nestedClosureParam", "$nestedVisible", "$nestedArrowParam"],
            &["$nestedArrow"],
        ),
        (
            "/*nestedClosure*/",
            &["$nestedArrowLayer", "$nestedClosureLayer"],
            &["$nestedClosureParam", "$nestedVisible"],
        ),
        (
            "/*unicode*/",
            &["$unicodeParam", "$unicodeLocal"],
            &["$unicodeAfter", "$functionParam"],
        ),
        ("/*incomplete*/", &["$incompleteParam"], &["$functionParam"]),
    ];

    for (index, (marker, expected, rejected)) in cases.iter().enumerate() {
        let (line, character) = marker_position(marker);
        let response = service
            .ready()
            .await
            .unwrap()
            .call(completion_request((index + 2) as i64, uri, line, character))
            .await
            .unwrap();
        let result = extract_result(response);
        let labels = completion_items_from_result(&result)
        ...
        for label in *rejected {
            assert!(
                !labels.iter().any(|candidate| candidate == label),
                "completion at {marker} must not contain {label}, got: {labels:?}"
            );
        }
    }
